    ...
    <div class="content-wrapper">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('client.demande.update', ['id' => $demande->id]) }}" method="POST">
                            @csrf
                            <div class="row">
                                 <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="reference">Référence <span class="text-danger">*</span></label>
                                        <input type="text" name="reference" class="form-control @error('reference') is-invalid @enderror" value="{{ $demande->reference }}" readonly>
                                        @error('reference')
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" value="{{ $demande->date }}">
                                        @error('date')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="desc" id="desc" class="form-control @error('desc') is-invalid @enderror">
                                            ...
                                            @foreach ($poubelles as $poubelle)
                                                <option value="{{ $poubelle->product }}" {{ $demande->desc == $poubelle->product ? 'selected' : '' }}>{{ $poubelle->product }} - @money($poubelle->price, 'XOF')</option>
                                            @endforeach
                                        </select>
                                        @error('desc')
                                            <span class="text-danger">{{ $message }}</span>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <select name="gie_id" id="gie_id" class="form-control  @error('gie_id') is-invalid @enderror">
                                            ...
                                            @foreach ($gies as $gie)
                                                <option value="{{ $gie->id }}" {{ $demande->gie_id == $gie->id ? 'selected' : '' }}>{{ $gie->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('gie_id')
                                            <span class="text-danger">{{ $message }}</span>
